...
def NomeTime(times: list[Jogo]) -> list[str]:
    ...
    ['Sao-Paulo', 'Palmeiras', 'Atletico-MG']
    '''
...
def AtribuiDadosTime(jogos: list[Jogo]) -> list[DadosTime]:
    ...
    [DadosTime(nome='Flamengo', pontos_totais=3, vitorias=1, saldo_gols=1, gols_sofridos=1, jogos_anfi=1, pontos_anfi=3), DadosTime(nome='Palmeiras', pontos_totais=0, vitorias=0, saldo_gols=-1, gols_sofridos=2, jogos_anfi=0, pontos_anfi=0), DadosTime(nome='Sao-Paulo', pontos_totais=0, vitorias=0, saldo_gols=-1, gols_sofridos=2, jogos_anfi=1, pontos_anfi=0), DadosTime(nome='Atletico-MG', pontos_totais=3, vitorias=1, saldo_gols=1, gols_sofridos=1, jogos_anfi=0, pontos_anfi=0)]
    '''

    dados_times = []
    nomes_times: list[str] = NomeTime(jogos)

    #duas funçoes, dados_times.append(nome, 0, 0, 0, 0, 0, 0)
    ...
    if time1.pontos_totais < time2.pontos_totais:
        ...
        if time1.vitorias < time2.vitorias:
            ...
            if time1.saldo_gols < time2.saldo_gols:
                ...
                nomes_ordenados = BubbleSort([time1.nome, time2.nome])
